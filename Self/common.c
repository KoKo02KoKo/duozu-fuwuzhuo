...
void main_task()
{
    ...
    if(DEBUG && (HAL_GetTick() - debug_last_tick >= 100))
    {
      ...
      printf("sh.angle=%d, sl.angle=%d\r\n", sh.angle, sl.angle);
      printf("sh.duty=%d, sl.duty=%d\r\n", sh.duty, sl.duty);
    ...
    if(debug_uart.is_received(&debug_uart))
    {
      ...
      strcmp((char*)debug_uart.rx_buf, "+") == 0 ? (sh.angle+=10) : 0; // 收到 "+" 则增加舵机角度
      strcmp((char*)debug_uart.rx_buf, "-") == 0 ? (sh.angle-=10) : 0; // 收到 "-" 则减少舵机角度
      // 收到 "DUTY=xxx" 则设置舵机占空比（手动解析，避免 sscanf 无堆时破坏内存）
      if(strncmp((char*)debug_uart.rx_buf, "DUTY=", 5) == 0)
      {
          int val = 0;
          char *p = (char*)debug_uart.rx_buf + 5;
          while(*p >= '0' && *p <= '9')
          {
              val = val * 10 + (*p - '0');
              p++;
          }
          sl.duty = (uint32_t)val;
      }
    }
    // web_uart 和 screen_uart 的数据处理逻辑可以根据实际需求来写，这里暂时只是清除接收标志
    if( servo_open == 0 && web_uart.is_received(&web_uart) && servo_open == 1)
    {
      web_uart.rx_flag = 0;
    }
    if(servo_open == 0 && screen_uart.is_received(&screen_uart))
    {
      screen_uart.rx_flag = 0; 
    }
    // 陀螺仪串口的处理逻辑是解析数据帧并更新 angle 结构体，具体解析函数在 angle.c 中实现
    if(servo_open == 0 && gyro_uart.is_received(&gyro_uart))
    {
